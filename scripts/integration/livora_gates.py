...
def _branch_evidence_from_meta(meta: dict) -> gite.BranchEvidence:
    ...
    # Legacy v1 meta.json has no merge_base / behind / ahead. The v1 gate job
    # checks out with fetch-depth: 0, so BOTH objects are local — derive the
    # topology from git here rather than failing closed on absent fields
    # (requirement 41: the v1 invocation contract keeps working on v2 code).
    if ((not ev.merge_base) or ev.commits_behind < 0) and ev.base_sha and ev.head_sha:
        try:
            chk = subprocess.run(["git", "rev-parse", "--git-dir"],
                                 capture_output=True, text=True)
            if chk.returncode == 0:
                if not ev.merge_base:
                    mb = subprocess.run(["git", "merge-base", ev.base_sha, ev.head_sha],
                                        capture_output=True, text=True)
                ...
                if ev.merge_base:
                    if ev.commits_behind < 0:
                        behind = subprocess.run(
                            ["git", "rev-list", "--count", f"{ev.merge_base}..{ev.base_sha}"],
                            capture_output=True, text=True)
                        if behind.returncode == 0:
                            ev.commits_behind = int(behind.stdout.strip() or 0)
                    if ev.commits_ahead < 0:
                        ahead = subprocess.run(
                            ["git", "rev-list", "--count", f"{ev.merge_base}..{ev.head_sha}"],
                            capture_output=True, text=True)
                        if ahead.returncode == 0:
                            ev.commits_ahead = int(ahead.stdout.strip() or 0)
        except OSError:
            pass
    if not ev.merge_base:
...
def run_classify(args) -> tuple[int, dict]:
    ...
    if args.no_git:
        ...
        if be is None:
            ...
            # CI reality: pull_request checkouts are DETACHED at the merge ref —
            # the lane branch ref legitimately does not exist locally even
            # though GitHub just proved the head SHA exists. Meta then carries
            # the existence/topology evidence.
            if "BRANCH_NOT_FOUND" in be.gaps and meta_be.branch_exists:
                be.gaps.remove("BRANCH_NOT_FOUND")
                be.branch_exists = True
                if not be.head_sha:
                    be.head_sha = meta_be.head_sha
            if "BASE_BRANCH_NOT_FOUND" in be.gaps and meta_be.base_sha:
                be.gaps.remove("BASE_BRANCH_NOT_FOUND")
                be.base_sha = meta_be.base_sha
            ...
            if not be.force_push_suspect and meta_be.force_push_suspect:
                be.force_push_suspect = True
