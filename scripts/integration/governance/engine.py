...
def classify(gi: GovernanceInput) -> GovernanceResult:
    ...
    # GitHub's raw enum leaks through legacy callers (v1 workflow lowercases
    # MERGEABLE/CONFLICTING instead of mapping) — normalize synonyms, keep
    # everything else fail-closed.
    mg = {"mergeable": "clean", "clean": "clean", "conflicting": "dirty",
          "dirty": "dirty"}.get(mg, mg)
    if mg == "dirty":
...
def compute_predicates(gi: GovernanceInput, res: GovernanceResult,
    ...
    return {
        ...
        "mergeability_clean": ({"mergeable": "clean", "clean": "clean", "conflicting": "dirty",
                                "dirty": "dirty"}
                               .get((gi.meta.get("mergeable") or "unknown").lower(),
                                    (gi.meta.get("mergeable") or "unknown").lower())
                               == "clean"),
