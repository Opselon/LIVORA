...
def test_subset_yaml_matches_pyyaml_on_shipped_registries():
    """The no-PyYAML fallback must produce the EXACT structure PyYAML gives for
    every registry shipped in this repo (master v1 + current v2) — otherwise a
    CI host without PyYAML would classify differently than a dev host."""
    import io as _io, subprocess
    yaml = pytest.importorskip("yaml")
    from governance.registry import _subset_yaml
    roots = [cf.REGISTRY_V1, cf.REGISTRY_LIVE]
    texts = [_io.open(x, encoding="utf-8").read() for x in roots]
    old = subprocess.run(["git", "-C", cf.REPO_ROOT, "show",
                          "master:.github/OWNERSHIP.yaml"],
                         capture_output=True, text=True)
    if old.returncode == 0:
        texts.append(old.stdout)
    for t in texts:
        assert yaml.safe_load(t) == _subset_yaml(t)


def test_subset_yaml_rejects_outside_grammar():
    """Anything the strict subset cannot parse must RAISE (fail closed), never
    guess: tabs, duplicate keys, anchors, block scalars, multi-doc, flow maps."""
    from governance.registry import _subset_yaml, SubsetYamlError
    for bad in ("version: 1\n\tpolicy: {}\n",
                ...
                "---\nversion: 1\n",
                "policy: |\n  multiline\n"):
        with pytest.raises(SubsetYamlError):
            _subset_yaml(bad)


def test_load_registry_without_pyyaml_is_deterministic_and_strict(monkeypatch, tmp_path):
    """Simulate a host with no PyYAML (sys.modules['yaml']=None -> ImportError
    on import): the v2 registry still loads via the strict-subset parser to the
    same lane/claim counts, and schema violations still come back RED — never
    a crash, never a permissive default. (No module reload: reload would
    rebind classes other test modules already imported, leaking across files.)"""
    monkeypatch.setitem(sys.modules, "yaml", None)
    reg = load_registry(cf.REGISTRY_LIVE)
    assert len(reg.lanes) == 15 and not reg.validation_findings
    bad = tmp_path / "OWNERSHIP.bad.yaml"
    bad.write_text('version: 2\nfrozen: "not-a-map"\n', encoding="utf-8")
    with pytest.raises(RegistryError) as ei:
        load_registry(str(bad))
    assert any(c.severity.value == "HARD_RED" for c in ei.value.constraints)