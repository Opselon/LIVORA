...
| Server CI job | **APPLIED — pending first enforcement** | the `server-gates` job landed in `.github/workflows/pr-gates.yml` (lead PR after v1.3.0; deltas from the proposal recorded in the `CI-SERVER-JOB.md` header) | `pull_request` runs use the BASE branch's workflow definition, so enforcement starts with the first PR merged after that lead PR; until then the backend stays gated by lane discipline only |
